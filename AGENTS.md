...
- **Labels**: All sidebar group labels MUST be defined in `src/content/nav/en.ts` and referenced via the `group()` helper in `config/sidebar.ts`.
...
1. Add the label key and value to `src/content/nav/en.ts`.
