...
2.  Create the `rehype-autolink` plugin:

    Create a new file named `rehype-autolink.ts` in the `plugins` directory:
...
3.  Configure Astro
    ...
    Import and configure the plugin in `astro.config.mjs`:
...
4.  Style the Headings

    Create a `headings.css` file in `src/styles/` and add the following styles:
